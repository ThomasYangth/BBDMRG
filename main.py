...
def do_DMRGX_and_save (mpo, k, chi_max=50, numsweeps=10, which = "SM", M=None, Mb=None, method="biortho", stop_if_not_converge = True, savename = None):
    ...
    _, _, _ = doDMRG_excited(M, Mb, mpo, chi_max, k, expected_gap=0.5, numsweeps=numsweeps, method=method, which=which, stop_if_not_converge=stop_if_not_converge, log_write=log_write, savename=savename)
...
if __name__ == "__main__":
    ...
    mpo = LindbladMPO(15, Operator({"ZZ":1, "X":0.5}), [Operator({"X":0.1,"Y":0.1j}), Operator({"Z":0.1})], dagger=True)
    #time_DMRG(mpo, which=-2+0j)
    #compare_DMRG_to_ED(mpo, -0.1+0j, chi_max=40, method="lrrho")
    #compare_DMRG_to_ED_X(mpo, 5, chi_max=50, numsweeps=5, which="SM", method="lrrho", stop_if_not_converge=False)
    do_DMRGX_and_save(mpo, 3, chi_max=50, numsweeps=10, which=0j, method="lrrho", stop_if_not_converge=False)
    exit()
